...
def analyze_by_nist_control(results):
    ...
    for control, stats in sorted(control_summary.items()):
        ...
        print(f"  {control}: {coverage:.1f}% compliant ({stats['passed']}/{stats['total']} checks passed)")
...
def check_critical_controls(results):
    ...
    for control in critical_controls:
        control_results = [r for r in results 
                          if control in r['nist_mappings']]
        
        if control_results:
            failed = sum(1 for r in control_results if r['status'] == 'FAIL')
            total = len(control_results)
            status = "✅ COMPLIANT" if failed == 0 else f"❌ NON-COMPLIANT ({failed}/{total} checks failed)"
